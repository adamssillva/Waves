...
class MatcherTestSuite extends MatcherSuiteBase {
  ...
  "Check cross ordering between Alice and Bob " - {
    ...
    val order1         = matcherNode.prepareOrder(aliceAcc, aliceWavesPair, SELL, 2.waves * Order.PriceConstant, orderVersion, 2.minutes)
    ...
    "sell order could be placed correctly" - {
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder = matcherNode.prepareOrder(bobAcc, aliceWavesPair, SELL, 300, (19.waves / 10.0 * Order.PriceConstant).toLong, orderVersion)
        ...
        val order3 = matcherNode.placeOrder(bobAcc, aliceWavesPair, SELL, 150, (19.waves / 10.0 * Order.PriceConstant).toLong, orderVersion)
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val order4 = matcherNode.placeOrder(aliceAcc, aliceWavesPair, SELL, 100, 2.waves * Order.PriceConstant, orderVersion)
        order4.status should be("OrderAccepted")
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = matcherNode.placeOrder(bobAcc, aliceWavesPair, BUY, 130, 2.waves * Order.PriceConstant, orderVersion)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        val order7 =
          matcherNode.placeOrder(aliceAcc, bobWavesPair, BUY, bobAssetQuantity, 1.waves * Order.PriceConstant, orderVersion)
